...
def test_routing_remote_claude_uses_claude_adapter() -> None:
    ...
    router = RoutingAgentAdapter(
        ...
        claude_adapter=lambda i, p: (
            calls.append("claude") or AgentResult(0, 1, False, "claude", "")
        ),
        remote_adapter=lambda i, p: (
            calls.append("remote-pi") or AgentResult(0, 1, False, "remote", "")
        ),
    )
