...
- **Input**: ADR-0042 (accepted 2026-07-18, grill on Podium #505) is live-verified: bridge CLI + UI Sync button + close-back + sync blocked_by sort fix shipped 2026-07-19; smoke ran GitHub #10 + children #12/#13 through `podium_issues.sync_from_github` → Podium issues #522/#523 → auto-land → SHAs `cf7b922` + `3b95a21` → close-back to GitHub via `gh issue close <n> --comment "Landed in <sha>."` Live bug discovered during the smoke (GH #13 blocked_by #12 dropped silently because `gh issue list` is newest-first and the dependent inserted before its blocker) was fixed same-session as commit `430e468` (symphony `fix(#514): sort sync children by number so blocked_by resolves in one pass`) with regression test `test_sync_maps_blocked_by_within_single_pass_newest_first`.
- **Change**: 4 new claims admitted via `gate.py check --apply` (IDs allocated sequentially by gate, starting from next-free after C-0383):
  - **C-0384 (decision)** — Insert-time `auto_land` provenance gate. The bridge decides `auto_land` at sync-INSERT time, gated on whether `scheduler._extract_runnable_verification` (scheduler/__init__.py:396) extracts a runnable command from the GitHub issue body's `## Verification` section. **Live-verified load-bearing format constraint:** only single-backtick inline form is recognized; triple-backtick fenced code blocks return empty → `auto_land=false` (silent surprise to operators pasting heredocs). Multiple inline commands chained with the word `and` join with ` && `. Reverses ADR-0023's `auto_land` provenance design from "slicer-only stamp" to "extractable runnable verification, slicer or otherwise."
  - **C-0385 (config-fact)** — `github:` external_id convention. The second documented `external_id` convention alongside `automation:<id>:loop`; both prefix-namespaced so lookups stay unambiguous. Nullable free-form TEXT with a UNIQUE index (web/api/schema.py:67+86) makes sync insert-only and idempotent. Close-back (the ONLY Podium→GitHub write) is centralized in `PodiumTrackerAdapter.transition_state` → `_maybe_close_back_github` (tracker_podium.py:460→503), covering all four STATE_DONE sites (scheduler/__init__.py:1566, 1805, 2290, 2482); fail-soft.
  - **C-0386 (gotcha)** — sync blocked_by ordering fix (commit `430e468`, 2026-07-19). `gh issue list` is newest-first; when a dependent (higher issue number) was listed before its blocker (lower number), the dependent inserted first and its `## Blocked by` edge lookup found no mirrored blocker → the edge was **silently dropped within a single sync pass**. Smoke: GitHub #13 (blocked_by #12) mirrored to Podium #522 with `blocked_by=[]` instead of `[#523]`. Both children still auto-landed (SHAs `cf7b922`, `3b95a21`) and closed back correctly — only the dependency edge was lost. Fix: `child_issues.sort(key=lambda item: int(item["number"]))`. Regression test `test_sync_maps_blocked_by_within_single_pass_newest_first` reproduces newest-first ordering. Full suite 1678 passed.
  - **C-0387 (gotcha)** — podium-web.service deploy gotcha (operator-judged warranted; the task spec offered C-0387 as the candidate ID). `podium-web.service` runs `pnpm start -p 8091` (serves the prebuilt `.next`) with no `ExecStartPre` build step. A frontend-only slice (e.g. #516 Sync-from-GitHub button) does not appear on the live UI until an explicit `cd web/frontend && pnpm build && sudo systemctl restart podium-web.service`. Complementary to the `deploy.sh` hazard cluster (C-0347/348/354) which covers in-deploy hazards; this gotcha covers the **slice-side miss** of not invoking deploy at all. Live-observed 2026-07-19: Sync button code shipped in `38501da` but UI did not render until out-of-band `pnpm build` + restart.
- **Outputs**: new analysis page `wiki/analyses/adr-0042-github-podium-dispatch-bridge.md` (covers all four claims with citations to ADR + code + tests + commit + smoke SHAs); new row in `wiki/index.md` Analyses table; new `## ADR-0042` section in `wiki/ROUTING.md` with full keyword list (bridge, sync, external_id, auto_land, close-back, sync blocked_by sort, podium-web prebuild-serve); four new eval cases appended to `wiki/eval/podium-api.eval` covering all four claims (all PASS, with one pre-existing FAILs cluster unrelated to this session left untouched per the surgical-changes rule).
- **Claims**: C-0384 (decision, ADMIT), C-0385 (config-fact, ADMIT), C-0386 (gotcha, ADMIT), C-0387 (gotcha, ADMIT). Initial kind names "convention" and "bugfix" were rejected by the gate (kind whitelist is {config-fact, decision, gotcha, runbook-step}); mapped to config-fact and gotcha respectively.
- **Cross-repo note (not a claim, captured in the analysis page)**: the `to-tickets` `## Verification` field is a **dotfiles-binding** change (delivered via dotfiles Podium issue #520, landed dotfiles commit `b349dfd` this session), **not** part of symphony #513-517. The bridge inherits the discipline from `to-tickets`'s existing prompt-renderer contract.
- **Deployment**: All work in this pass is wiki-only — no service restart, no DB write, no Podium Issue mutation. The C-0386 fix already shipped as commit `430e468`; the ADR-0042 itself was `accepted` 2026-07-18 and deployed prior to this wiki pass.
- **Verification**: `gate.py check --apply` admitted all 4 claims cleanly (each in sequence, the gate allocated C-0384 → C-0387); `gate.py audit` shows 71 active claims against the 320 budget (well under cap); `gate.py eval` shows 25/29 cases pass including all 4 new cases (the 4 pre-existing FAILs are unrelated ADR-0026/0029 corpus drift cases).
- **Safety**: no service restart, no DB write, no service env-file read, no live Issue mutation; only wiki files (CLAIMS.md, index.md, ROUTING.md, log.md, new analysis page, new eval cases) touched. Working tree contains the uncommitted changes — parent will commit.
...
- **Input**: Podium Issue 464 (`mac-dotfiles`) failed twice after selecting `pi-duo/Duo`.
- **Root cause**: Symphony correctly validated and dispatched the outer virtual model, but remote Duo's nested actor selected `minimax/MiniMax-M3`; the noninteractive SSH runtime lacked the provider credential and its safe model inventory did not contain that model id. Pi Duo's picker metadata path catches actor lookup failure and still registers `Duo`; request-time `lookupModel(config.actor)` fails loud. The concurrent `zai/glm-5.2` warning was secondary enabled-model drift, not Duo's verifier path.
- **Evidence**: Issue/Run rows in `podium.db`; `runs/2687.log`; `runs/2692.log`; bounded scheduler journal around 2026-07-17 14:16 UTC; `/home/james/dotfiles/.pi/agent/extensions/pi-duo/extensions/pi-duo.ts`; non-secret remote model-id/preflight probes.
- **Output**: `wiki/raw/sessions/2026-07-17-issue-464-remote-pi-duo-runtime-gap.md`; low-risk maintenance section in `wiki/analyses/podium-issue-dispatch-contract.md`; index/routing updates.
- **Claim**: C-0380 (gotcha, ADMIT) — composite providers require nested actor/verifier readiness checks on each remote host; outer catalog validation is insufficient.
- **Maintenance**: scheduled gate consolidation passed 21/24→21/24 and marked stale C-0334 (the former `deepseek-v4-pro` Pi default) superseded by C-0368 (`Duo` default); active claims 68→67 and `maintenance_due=false`.
- **Safety**: no credentials or credential values captured; no service, DB, Issue, binding, remote config, or code mutation performed.
- **Open**: repair remote Duo's actor tuple and noninteractive credential availability, or requeue with a directly available CLIProxy model; add a nested-model remote preflight.

## [2026-07-16] session-update | patrol default model bare-name trap + backfill (issue #413, C-0373/C-0374)

- Actor: agent (Claude), operator-directed via /symphony-troubleshooter + grill-me on issue #413 (initial "why deepseek-v4-flash" question pivoted into "why no patrol issues since 2026-07-14 dispatch" once the live DB query showed the actual broken rows).
- **Input**: operator-reported dispatch-gate error `Dispatch blocked: model resolution failed: model 'pi-duo' is not in models.yml for agent pi; add it to the catalog or clear preferred_model`. The "actual model in pi is `pi-duo/Duo`" hint from the operator's steer line pointed straight at the slash-split contract.
- **Root cause**: `PATROL_DEFAULT_MODEL = "pi-duo"` (bare provider string, set by C-0368 on 2026-07-14). `model_catalog.resolve_model()` accepts exact-id OR `provider/id` form, but `"pi-duo".partition("/")` yields `("pi-duo", "", "")` — the empty `wanted_id` causes the `if wanted_id:` guard to skip the slash-split branch entirely, so the resolver falls through to `ModelResolutionError`. Three consecutive default-value flips in a row (`deepseek-v4-flash` C-0357, `Fusion Fast` C-0366, `pi-duo` C-0368) shipped working-or-broken depending on whether the value was an exact `entry["id"]` match — the bare-provider pattern was a previously-unwritten trap.
- **Fix** (symphony `1220493`): (1) `web/api/main.py:1007` `PATROL_DEFAULT_MODEL = "pi-duo/Duo"` (provider/id form), comment cites issue #413; (2) alembic migration `0020_patrol_issues_force_pi_duo.py` — `UPDATE issue SET preferred_model='pi-duo/Duo' WHERE origin='patrol' AND preferred_model='pi-duo'`, mirrors `0019`'s shape (data-only, irreversible, no-op downgrade); (3) the two `test_patrol_origin_*_pi_duo*` assertions updated; (4) new `test_0020_backfills_patrol_issues_to_pi_duo` covers the migration end-to-end.
- **Verification**: 64 `test_issue_create.py` + 3 `test_alembic_baseline.py` + 14 `test_dispatch_gate.py` + 16 `test_model_catalog.py` + 228 `test_scheduler.py` = 325 tests green. Live DB shows 8+ homelab patrol issues now pinned to `pi-duo/Duo` (after backfill) instead of the broken `pi-duo` — issue #407 alone had 30+ identical blocking comments before the fix.
- **Claims**: C-0373 (config-fact — the fix + backfill, ADMIT), C-0374 (gotcha — resolver bare-name trap + suggested near-miss error messages + tuple-identity regression-guard pattern for the next default flip, ADMIT). C-0368's Notes column extended with the bare-name bug context (catalog entry change still correct, only the constant value was wrong).
- **Index/ROUTING**: dispatch-contract row date bumped (2026-07-14 → 2026-07-16) + summary extended with the C-0373/C-0374 paragraph; analysis page `analyses/podium-issue-dispatch-contract.md` gained a "Patrol default model: bare-name trap + fix (2026-07-16)" section with full root-cause + regression-guard pattern; new raw session capture `wiki/raw/sessions/2026-07-16-patrol-default-model-bare-name-trap.md`.
- **Deploy**: `alembic upgrade head` + `podium-api.service` restart (NOT `symphony-host`, which is the scheduler). Live verification post-deploy: `SELECT id, state, preferred_model FROM issue WHERE origin='patrol' AND preferred_model='pi-duo'` returns empty; new patrol issues dispatch as `pi-duo/Duo`.
- **Follow-up (recommended for next slice)**: C-0374's tuple-identity regression-guard test + the two near-miss error messages in `resolve_model()` — prevents the same class of bug from re-appearing on the next default-value flip, and gives a more actionable error when a future maintainer authors a bare provider string.

## [2026-07-17] session-update | patrol legacy migration (deepseek-v4-flash → pi-duo/Duo, 11 issues, issue #413 follow-up)

- Actor: agent (Claude, slice), operator-directed via issue #413 comments (operator reply 2026-07-17T08:56:26 UTC "Yes migrate now" after seeing deepseek-v4-flash dispatches on the same homelab patrol rows across multiple morning ticks).
- **Input**: 10 → 11 pre-C-0368 homelab patrol issues still pinned to `deepseek-v4-flash` (63, 64, 65, 66, 67, 69, 71, 75, 76, 78, 341, 342 — created 2026-06-20 through 2026-07-09); the blocked-reconciler kept reopening them and the dispatcher re-fired them on `provider=deepseek model=deepseek-v4-flash:high` every cycle. The session-1 turn-1 recommendation ("leave them on the cheaper model — `deepseek-v4-flash` is the cheapest catalog entry") was overridden; operator chose uniformity on `pi-duo/Duo` over the per-token cost saving.
- **Change**: direct `UPDATE issue SET preferred_model='pi-duo/Duo' WHERE binding_name='homelab' AND origin='patrol' AND preferred_model='deepseek-v4-flash' AND state NOT IN ('done','archived')` against `podium.db`. **No alembic migration**: C-0373 migration 0020 only backfilled the bare-provider trap, not legacy rows; doing both as one direct UPDATE avoids a fourth alembic revision for a one-shot data fix. The affected issues re-pick up `pi-duo/Duo` on their next dispatch (state="blocked" rows wait for blocked-reconciler; state="in_review" rows carry the new model forward immediately).
- **Verification**: `SELECT id, state, preferred_model FROM issue WHERE binding_name='homelab' AND preferred_model='deepseek-v4-flash' AND state NOT IN ('done','archived')` returns empty (zero broken-state rows); the 11 affected rows now show `preferred_model='pi-duo/Duo'`. `SELECT binding_name, COUNT(*) FROM issue WHERE origin='patrol' AND preferred_model='deepseek-v4-flash' GROUP BY binding_name` returns only `homelab` (no other bindings have legacy rows — migration scope stays bounded).
- **Claims**: C-0375 (decision, ADMIT) — operator-initiated legacy migration. Records the operator's reversal of the session-1 turn-1 guidance, the direct-UPDATE rationale (no alembic revision), and the bounded scope (homelab only).
- **Index/ROUTING**: dispatch-contract analysis page gain a brief follow-up paragraph in the existing C-0373 section noting that legacy pre-C-0368 rows required a separate operator-initiated migration (C-0375); ROUTING keywords gain `legacy deepseek-v4-flash migration`, `podium.db direct UPDATE`, `C-0375`.
- **Deploy**: no service restart — direct DB write; the existing `podium-api.service` re-reads `preferred_model` from the row at dispatch time (per the C-0311 lesson "models.yml is re-read on every dispatch"; the analogous fact for `preferred_model` is true at the `tracker_podium._row` read path).
- **Not superseded**: C-0373 (the constant+migration fix for the bare-provider trap — that was a code fix; this C-0375 is a one-shot data fix for legacy rows); C-0368 (catalog change — still correct).
- **Reverses**: session-1 turn-1 guidance ("I'd recommend leaving them unless the cost is a concern").
- **Unresolved**: none for #413. C-0374 follow-up (near-miss error messages + tuple-identity regression guard test) still recommended for a future slice.

## [2026-07-15] session-update | harness gates re-established (advisory posture, global safety gates, Pi precedence)## [2026-07-15] session-update | harness gates re-established (advisory posture, global safety gates, Pi precedence)
...
- **Input**: operator noted an unstage from earlier in the conversation removed `Fusion`/`Fusion Fast` from `models.yml` (the live `PATROL_DEFAULT_MODEL = "Fusion Fast"` constant was now stale against the catalog); operator asked to add `pi-duo/Duo` and make it the per-agent `pi` default (global) AND the patrol default (global, via the same `PATROL_DEFAULT_MODEL` constant — there is no per-binding `default_model`). Provider = `pi-duo`, model id = `Duo` (both new to the codebase; no prior presence).
- **Change**: `models.yml` registers one new entry `{id: Duo, agent: pi, provider: pi-duo, default: true}`; the `Fusion` and `Fusion Fast` `pi-moa` entries remain absent (deleted earlier in this session). The per-agent `pi` default flips `deepseek-v4-pro` → `Duo`. `web/api/main.py` `PATROL_DEFAULT_MODEL = "pi-duo"` (was `"Fusion Fast"`); comment above the override also flipped from `deepseek-v4-flash` prose to `pi-duo` prose to match. Issue-create patrol tests renamed + asserted against the new value (`test_patrol_origin_defaults_preferred_model_to_pi_duo`, `test_patrol_origin_forces_pi_duo_over_pinned_model`); `test_operator_origin_leaves_preferred_model_unset` unchanged (operator issues still store a null `preferred_model`).
- **Verification**: `web/api/tests/test_issue_create.py` (61 passed), full `web/api/tests/` (334 passed, 1 skipped), `tests/skills/test_catalog_maintenance_skills.py` (7 passed). `_load_models` returns 13 entries; `pi` default is `Duo`; no entry contains `fusion`. **`pi --provider pi-duo --model Duo` is NOT smoke-tested in this session** — host provider registration is unverified (the `pi-duo` provider is new to this repo and not present in any tracked file). First patrol or first null-`preferred_model` operator issue will dispatch-block on a provider-resolution error if the host's `pi` CLI doesn't know about it.
- **Claims**: C-0368 (config-fact, ADMIT) supersedes C-0366's `PATROL_DEFAULT_MODEL` value (`Fusion Fast`→`pi-duo`) and the `deepseek-v4-pro`-as-`pi`-agent-default sub-claim of C-0142/C-0169 (`Duo` takes the slot). C-0366 and C-0357 marked `superseded`; the C-0357 notes column gained a third-supersession annotation citing C-0368. Mechanism (origin-scoped patrol force at `create_binding_issue`) intact across all three value flips.
- **Index/ROUTING**: dispatch-contract row date bumped (2026-07-11 → 2026-07-14); patrol-default paragraph extended to mention the dual-scope flip and the `pi --provider pi-duo --model Duo` runtime pre-condition; analysis paragraph 94 in `analyses/podium-issue-dispatch-contract.md` rewritten around `pi-duo`/`Duo`; ROUTING keywords extended with `pi-duo`, `Duo`, `pi-duo/Duo`, `PATROL_DEFAULT_MODEL pi-duo`, `per-agent pi default Duo`, `patrol posts as pi-duo`, `C-0368`, `dual-scope patrol+per-agent default flip`, `provider pi-duo`.
- **Deploy**: `podium-api.service` restart (create endpoint lives in the API); the `symphony-restart` skill was NOT called in this session — calling it is the operator's call. Skipped the implicit `podium-api` restart since this session is operator-driven via `pi`/`claude` interactive and the operator may want to stage the restart alongside the host-provider smoke.
...
## [2026-07-13] session-update | ADR-0037 natural turn on every terminal branch (C-0367)

- **Task**: grill-me on issue 379 symptom — completion comments routinely terse/empty/fallback, forcing run-history archaeology; operator also noted posted output differs from the CLI. Root-caused: no "SYMPHONY_COMPLETE" system exists; the ADR-0022 `_capture_natural_turn` was wired into `_classify_terminal` at ONE point, so timeout/exit/permission-gate/approval-gate/malformed-schedule, the `SYMPHONY_SCHEDULE` branch (overwrote it — run 1656/issue 67), and the three retry-exhaustion blocks fell back to `_extract_summary` → terse stub, dropping prose to `runs/<id>.log`. Two independent `pi -p` (deepseek-v4-pro) fact-checks confirmed the single-capture-point gap and the retry-helper terminal-block gap.
- **Change**: `scheduler/__init__.py` — hoisted `_capture_natural_turn(...)` to the TOP of `_classify_terminal` (before the retry gate); diagnostic-comment branches append the turn to `msg` when prose exists; schedule branch posts notice+turn; `_block_retry_ceiling`/`_maybe_retry_stall`/`_maybe_transient_review_retry` gained an optional `summary` param used only in their terminal-blocked branches; `_extract_summary` stays fallback only when the turn is empty. Re-dispatching retry branches unchanged. Comment ≠ CLI (pi capture is `text_delta`-only) is out of scope — only the dropped-prose half fixed.
- **Tests**: flipped 2 tests encoding the old omit-stdout decision; added natural-turn assertions to the schedule + two retry-exhaustion tests; revert-probe confirmed each fails pre-fix. 239 scheduler+transient-retry tests pass.
- **ADR**: `docs/adr/0037-surface-natural-turn-on-every-terminal-branch.md` (`accepted`), extends ADR-0022.
...
## [2026-07-17] session-update | podium-auto-migrate on boot, closing the schema-drift 500 (C-0376)

- **Task**: operator reported `POST /api/auth/login` returning HTTP 500 after an OS reboot; conversation surfaced the red herring that the password had been "wrong" every reboot, when in fact `podium-api.service` was crash-looping with `RuntimeError: Podium DB schema drift: missing columns [...]` because Alembic migration `0023_automation_pin_fields` (added 2026-07-17) had been checked into `web/api/migrations/` but never `uv run alembic upgrade head`'d against `/home/james/symphony/podium.db`. `journalctl -u podium-api.service` from 13:01 showed restart counter at 285+ before intervention; `symphony-host.env` `PODIUM_PASSWORD_HASH` was unchanged since 2026-07-04, so the password was never the problem.
- **Resolution**: `uv run alembic upgrade head` applied `0022 → 0023_automation_pin_fields` against `/home/james/symphony/podium.db` (alembic_version advanced, six pin columns landed). Pre-migration backup retained at `/home/james/symphony/podium.db.pre023.bak.20260717-131503`. Operationally: installed `[email protected]` (oneshot, RemainAfterExit=yes, calls `/home/james/symphony/.venv/bin/alembic upgrade head` from `WorkingDirectory=/home/james/symphony` with `EnvironmentFile=/home/james/symphony-host.env`) + drop-in `/etc/systemd/system/podium-api.service.d/migrations.conf` (`Wants=` + `Before=`). Tracked installer: `scripts/install-podium-migrations-service.sh` (idempotent, `set -euo pipefail`, requires root, refuses to run if alembic binary is absent, does not restart any service automatically). `podium-migrations.service` deliberately does NOT wire `OnFailure=telegram-alert@%n.service`; a failed migration propagates as `podium-api` crashing on `ensure_schema`, which fires the existing `OnFailure` alert. Two alerts for one root cause would be noise. Strict `ensure_schema` policy preserved unchanged (origin: 2026-06-12 stamp-vs-run drift incident; `web/api/main.py:540-543`). Smoke test: dropped a transient `_TEMP_smoke_0024.py` migration, restarted via `systemctl start podium-api.service` only — auto-heal pulled in, alembic advanced, `sim_marker` column landed, `/api/health` 200, `/api/auth/login` 401 (not 500); temp file removed, alembic_version stamped back to `0023_automation_pin_fields`. `podium-change-password.sh` (C-0132) and `web/cli/podium set-password` untouched.
- **Evidence**: `wiki/raw/sessions/2026-07-17-podium-schema-drift-auto-migrate.md` — session capture with journal excerpts and decision rationale. `wiki/raw/podium-migrations.service` and `wiki/raw/podium-api.service.d-migrations.conf` — live `systemctl cat` snapshots. `web/api/main.py:534-585` — `ensure_schema` (the original guard preserved). `web/api/migrations/versions/0023_automation_pin_fields.py` — the migration that triggered the incident. Smoke evidence: `journalctl -u podium-api.service` 2026-07-17 13:01–13:21.
- **Wiki**: immutable source `wiki/raw/sessions/2026-07-17-podium-schema-drift-auto-migrate.md`; raw unit snapshots `wiki/raw/podium-migrations.service` + `wiki/raw/podium-api.service.d-migrations.conf`; candidate analysis `wiki/candidates/analysis-session-podium-schema-drift-auto-migrate.md` (pending lint to promote to `wiki/analyses/`); updated `wiki/analyses/podium-018-auth.md` with a "Boot Ordering" subsection (low-risk maintenance: frontmatter `updated: 2026-07-17` + 4 added sources + tag additions + appended subsection); updated `wiki/sources/podium-systemd-units.md` with a 2026-07-17 live-update paragraph + frontmatter bump; root `wiki/index.md` Candidate Review Queue row appended (status: pending lint → promote to `analyses/`); `wiki/ROUTING.md` operations keyword line extended with `podium-migrations.service`, `podium-api.service.d-migrations.conf`, `install-podium-migrations-service.sh`, `alembic upgrade head`, `ensure_schema`, `schema drift`, `missing columns`, `500 on /api/auth/login`, `C-0376`. C-0376 admitted through `gate.py check --apply` as a `runbook-step` row (`writes_since_maint=19`).
- **Deploy**: shipped. `[email protected]` enabled; the next OS reboot will run `alembic upgrade head` before `podium-api.service` starts. No operator action required to extend the auto-heal; for an unrelated future migration that takes non-trivial wall-clock time, revisit `TimeoutStartSec=` (carried as out-of-scope follow-up).
- **Verification**: `systemctl is-active podium-migrations.service` → active; `systemctl is-active podium-api.service` → active; `curl http://127.0.0.1:8090/api/health` → HTTP 200; `curl -d '{"password":"wrong-test"}' http://127.0.0.1:8090/api/auth/login` → HTTP 401 `{"detail":"invalid password"}` (no more 500). `sqlite3 /home/james/symphony/podium.db "SELECT version_num FROM alembic_version;"` → `0023_automation_pin_fields`. Idempotent installer re-run reconverged the units without restarting any service.
- **Unresolved**: (a) `podium-backup.sh` (cron-daily 02:17) does not currently coordinate with migrations; a backup taken mid-migration could capture a torn schema. Today's window is far from boot (nightly), so practical risk is small — revisit only if a future migration proves non-transactional. (b) `podium-migrations.service` journal is silent on the no-op path; an operator wanting to verify "did it run?" reads `systemctl status` (Result / Exit Code / Activated timestamps), not the journal. (c) Candidate analysis page still in `wiki/candidates/` — next lint pass will promote to `wiki/analyses/`. (Interactive operator-driven wiki pass, not a slice run, ADR-0028 exemption N/A.)

## [2026-07-17] session-update | call-graph trace through `_classify_terminal` (C-0377/C-0378/C-0379)

- **Task**: mapped the function-level call graph from `main.run_bindings_loop` down to `session_continuity.derive_session_id`, then audited `scheduler._classify_terminal` for the repeated blocked-terminal pattern. Original graphify AST graph over-counted cross-module callers (e.g. `derive_session_id` had 38 inflated, real is 5); produced a focused cross-module call graph (178 Python files, 8,418 resolved call edges, 47% cross-module ratio) to ground the trace.
- **Findings**: (1) full dispatcher spine is 7 functions / 6 hops / 7 modules — `main.run_bindings_loop` → `scheduler.loop.run_loop` → `scheduler._dispatch_one` → `scheduler.tick.run_tick` → `scheduler.tick._prepare_run_tick_dispatch` → `scheduler._prepare_resume_candidate` → `session_continuity.derive_session_id`. (2) `derive_session_id` has 5 production callers (4 dispatcher + 1 live-tail): `agent_runner.run_remote_agent:631`, `agent_runner.run_pi_rpc_agent:904`, `claude_runner.run_claude_agent:810`, `scheduler._prepare_resume_candidate:285`, `web.api.main._SessionTailer.._poll_running:311`; the two sides share `derive_session_id` as a leaf with zero upstream ancestors in common. (3) `web.api.main.lifespan()` calls `scheduler.loop.run_loop()` directly — FastAPI process owns the binding loop. (4) `_classify_terminal` is a 270-line async decision cascade with 33 callees, 17 `TickResult` reasons, 13 parameters; 6 branches (`timeout`, `nonzero`, `permission-gate`, `approval-gate`, `agent-marker-blocked`, both `agent-scheduled-malformed` branches) share a structurally-identical blocked-terminal block. (5) An `_emit_blocked_terminal` extraction is in flight in `scheduler/__init__.py:1073` (4 of 6 sites converted as of 2026-07-17 14:18 UTC; remaining: `agent-marker-blocked`, both `agent-scheduled-malformed` branches).
- **Not in scope**: did NOT author the in-progress `_emit_blocked_terminal` refactor — observed it in the working tree at session start (parallel slice, unknown owner). Did NOT commit a custom AST call-graph extractor — threw it away after the analysis (advisor flagged a simple-name fallback correctness concern; live source citations are the durable artifact).
- **Evidence**: `wiki/raw/sessions/2026-07-17-call-graph-trace-classify-terminal.md` (immutable session capture); live source citations at `scheduler/__init__.py:285, 1073-1660`, `scheduler/tick.py:568`, `scheduler/loop.py`, `main.py`, `agent_runner.py:631,904`, `claude_runner.py:810`, `session_continuity.py:34`, `web/api/main.py:311`, `tests/test_scheduler.py`. Throwaway `.graphify_call_graph.py` AST extractor deleted; `graphify-out/` added to `.gitignore`.
- **Wiki**: candidate `wiki/candidates/analysis-session-call-graph-classify-terminal.md` (status: pending lint → promote to `analyses/`); C-0377 (config-fact: 5 production callers), C-0378 (config-fact: blocked-terminal pattern + 410 LOC of structurally-identical code), C-0379 (config-fact: `lifespan()` calls `scheduler.loop.run_loop()`) all admitted through `gate.py check --apply`. `wiki/index.md` candidate queue row appended; `wiki/ROUTING.md` Architecture keyword line extended. Audit `gate.py audit` clean (66 active / 320 budget).
- **Open**: (a) `_emit_blocked_terminal` refactor is partial — 2 of 6 sites still inline; whoever finishes it (likely the parallel slice that started it) saves ~38 more LOC. (b) The `.gitignore` change adding `graphify-out/` is part of this commit but is independent of the wiki update — separate concern but trivial review surface.
- **Pre-existing dirty working tree**: `bindings.yml`, `plans/.patrol-incident-dedup-and-bounded-history.state.yml`, `web/frontend/app/[binding]/automations/page.tsx`, `web/frontend/tests/automations.spec.ts`, `scheduler/__init__.py` (mid-`_emit_blocked_terminal` extraction by a parallel slice) — NOT touched by this commit; left for the owning slices.
- **Verification**: `gate.py audit` exits 0; `wiki/candidates/analysis-session-call-graph-classify-terminal.md` has frontmatter (with `type`) + sources + confidence + tags, and is OKF-conformant (bundle-relative markdown links, `# Citations` section). Raw session capture path `2026-07-17-call-graph-trace-classify-terminal.md` is new (no overwrite). All 3 claim rows cite live source paths. No secrets, no raw pasted user content captured. (Interactive operator-driven wiki pass, not a slice run, ADR-0028 exemption N/A.)

## [2026-07-17] session-update | reconciled with parallel slice #465 `_emit_blocked_terminal` land

- **Update to the previous log entry:** the parallel slice landed at `3396c7a refactor(#465): extract _emit_blocked_terminal helper` at 2026-07-17 14:21 UTC (3 minutes after this session's analysis observed the partial extraction). The committed refactor converts all 7 sites in `_classify_terminal` (lines 1259, 1282, 1333, 1366, 1419, 1446, 1470), net diff -137 LOC, +136 LOC; saves ~137 LOC of structurally-identical code. The candidate analysis page and the raw session capture have been updated to reflect this — both now reference `3396c7a` and the 7-site count.
...
## [2026-07-23] session-update | finish-spec verification for Podium issue chat #37

- **Task**: walked the locked acceptance contract in `docs/handoffs/2026-07-21-018-podium-issue-chat-spec.md` after implementation children #30–#36 closed. Backend and TypeScript gates are green; focused issue-chat Playwright coverage is 38/38 after correcting one stale `"PI"` fixture expectation to the current `"pi"` value.
- **Integration fix**: commit `934ff60` adds `scheduler.stamp._stamp_agent_comment()` and routes scheduler completion/question/review-reconcile writes through it, so output from `CandidateIssue.origin == "patrol"` is stamped `patrol` instead of `agent`. Focused scheduler verification passed 248 tests; the complete backend rerun passed 1694 with 2 skipped.
- **Closure verdict**: #37 remains open. The complete Playwright run is red (95 passed, 15 failed, 1 not run), and spec §6's required streaming-feel choice is not implemented. Follow-ups #38–#41 cover fixture/create/schedule drift, binding/filesystem isolation, Inbox/WebSocket signal stability, and rAF-batched live-tail rendering. Status was recorded on GitHub issue #37; no completed child was modified.
- **Wiki**: updated `wiki/concepts/operator-reply.md` and its root-index row with the landed uniform role-stamp contract and patrol-origin correction. No candidate or new claim was added because the parent feature is not accepted yet; the existing C-0285 wording that `/comment` is byte-verbatim/no-header is now historical attribution behavior and should be superseded or amended when #37 closes. The unrelated pre-existing schema-drift candidate remains untouched.
- **Evidence**: commit `934ff60`; `scheduler/stamp.py`; `scheduler/reconcile.py`; `tests/test_scheduler.py`; `web/frontend/tests/new-issue.spec.ts`; `docs/handoffs/2026-07-21-018-podium-issue-chat-spec.md`; GitHub #37 and subissues #38–#41. No raw session capture was needed because code, tests, commit, spec, and tracker comments are the primary evidence.
- **Unresolved**: close #38–#41, rerun the full backend/TypeScript/Playwright gates, then rerun finish-spec and close #37 only if every acceptance criterion passes. (Interactive operator-driven wiki pass, not a Podium slice; ADR-0028 exemption N/A.)

## [2026-07-23] session-update | GitHub issue #38 fixture/create/schedule contract repair

- **Task**: Playwright fix for issue #38 (parent #37). The five affected specs (`dependency-chip`, `editing`, `schedule`, `session-tail`, `skill-catalog`) drifted from three current contracts: NULL-safe scoped skill upsert (ADR-0033 + 0016), description-required `IssueCreate` (#138), and staged-on-Send flyout dispatch with post-create flyout auto-open (F4, #036/#084).
- **Repairs**: commit `7362fe3`. `web/frontend/tests/fixtures.ts` `seedSkills` ON CONFLICT target mirrors `ux_skill_scope(name, host, IFNULL(binding_name, ''))`; bare `(name, host, binding_name)` never fires for host-global rows. `dependency-chip.spec.ts` switches `title`→`description` on every `POST /api/bindings/{name}/issues` body (the `IssueCreate` model has no `title` field). `schedule.spec.ts` fills the description composer and accounts for the F4 flyout auto-open; the flyout schedule tests now stage `Yes`/`No` and click the composer Send (which applies `POST`/`DELETE /schedule` and closes the flyout via `onSent`) instead of asserting immediate dispatch on select. `session-tail.spec.ts` first test seeds a runless issue via `seedIssue` rather than clicking the seed `Seed todo issue for homelab` (which carries a succeeded seed run and now renders the terminal bubble instead of the empty placeholder).
- **Verification**: `pnpm exec playwright test tests/dependency-chip.spec.ts tests/editing.spec.ts tests/schedule.spec.ts tests/session-tail.spec.ts tests/skill-catalog.spec.ts --workers=1` → 12 passed (1.7m) against a fresh E2E database. No product code changed; contract drift only.
- **Wiki**: new claims C-0399 (`IssueCreate` payload contract: description-required, server-generated title, `schedule: ScheduleRequest` wrapper, origin precedence, patrol-model force) and C-0400 (F4 new-issue modal auto-open on create success, `onCreated` indirection, two-stage attachment release ordering). No candidate page needed; the durable facts are claims about current code, not new concepts.
- **Evidence**: commit `7362fe3`; `web/frontend/tests/fixtures.ts`; `web/frontend/tests/dependency-chip.spec.ts`; `web/frontend/tests/schedule.spec.ts`; `web/frontend/tests/session-tail.spec.ts`; GitHub issues #37 and #38. Playwright runs against `PODIUM_DB_PATH=test-results/podium-e2e.db` per `playwright.config.ts`.
- **Resolution**: pushed `7362fe3` + `c1faaf1` to origin/main; closing comment + status flip done (issue closed 2026-07-23 03:41:52Z).
## [2026-07-23] session-update | Correct wiki diff to full issue-chat implementation closure status

- **Task**: surgical wiki maintenance — the prior session left `wiki/concepts/operator-reply.md`, `wiki/index.md`, the new raw capture, and this log entry drifting out of sync with the actual state of the GitHub issue chain. The implementation children (#30–#36) and acceptance follow-ups (#38–#41) and parent (#37) are all CLOSED; the F1 bubble-rendering slice is the live `podium-web` build. The previous wording also mis-attributed #19/#20/#24/#25/#28/#29 as "remaining spec slices" — those are Wayfinder decision/provenance tickets, not implementation work — and mis-stated that the previous bundle was "rolled back" (it is retained for a manual rollback option) and that a public Podium probe was blocked because the repo is private (only public GitHub issue verification was unavailable; authenticated `gh` grounded issue states).
- **Decision**: update the leading status paragraph on `wiki/concepts/operator-reply.md` to record the actual implementation closure (children #30–#36 + follow-ups #38–#41 + parent #37 closed; F1 frontend deployed with `BUILD_ID = wgN3tmVXMyH-QwuqCFgpP`, previous bundle retained at `web/frontend/.next.prev/BUILD_ID = vUsX0hEgqsiTSUPbAhhW2`; HTTP 200 on `:8091`; clean tree); drop the brittle line-number claims about `IssueChat` import/usage; keep the precise independence note ("public GitHub issue verification was not available; authenticated `gh` grounded the states"). Restore `wiki/CLAIMS.md` from HEAD after `gate.py migrate` accidentally dropped the Notes content for C-0399/C-0400. Do **not** add a candidate page, claim, or route change — closure/status is page maintenance, not a load-bearing atomic fact (the underlying slice work is already cited at C-### level). No new claim is admitted; no open questions remain about the closed-and-deployed state.
- **Files**: `wiki/raw/sessions/2026-07-23-podium-issue-chat-deploy.md` (rewritten to match the corrected state); `wiki/concepts/operator-reply.md` (leading status paragraph + new `Independence note`); `wiki/index.md` (root Concepts row summary); this log entry. `wiki/CLAIMS.md` restored from HEAD (`git checkout -- wiki/CLAIMS.md`). `wiki/ROUTING.md` deliberately untouched (route unchanged).
- **Verification**: `gate.py audit` exits 0; `git diff wiki/CLAIMS.md` is empty (restored verbatim from HEAD); a regex sweep across the four edited wiki records returns no active false-status statements (no current text labelling any of #19/#20/#24/#25/#28/#29 as unbuilt slices, and no current text labelling #37 as open); the new raw capture accurately lists #30–#41 closure; only the four intended wiki content files plus the new raw capture differ (plus `wiki/CLAIMS.md` restored = no diff); secret-shape scan over the changed files found no `api_key|token|secret|password|BEGIN PRIVATE KEY|AWS_SECRET_ACCESS_KEY` matches.
- **Deploy/ground evidence**: `gh issue view 30 … 41 --json state,title` returns CLOSED for all twelve issues; `systemctl is-active podium-web.service` → `active`; `systemctl status podium-web.service` → `active (running) since Thu 2026-07-23 13:56:18 UTC`; `web/frontend/.next/BUILD_ID` = `wgN3tmVXMyH-QwuqCFgpP`; previous `vUsX0hEgqsiTSUPbAhhW2` retained at `web/frontend/.next.prev/BUILD_ID`; `curl -sS -o /dev/null -w "HTTP %{http_code}\n" http://10.20.20.16:8091/` → `HTTP 200`; `git status --short` clean post-deploy; `web/frontend/components/IssueFlyout.tsx` imports and renders `web/frontend/components/IssueChat.tsx` (whose file header comment confirms F1 ownership).
- **Independence note (recorded)**: public GitHub issue verification was not available; issue-state evidence is grounded in authenticated `gh issue view 30 … 41 --json state,title,closed,closedAt` on the operator-configured `github-personal` host alias. The local evidence (active unit, HTTP 200, source wiring on `main`, BUILD_ID diff, clean tree) is unaffected by the public-probe gap.
- **Wiki-session scope**: this is an interactive operator-driven wiki maintenance pass (ADR-0028 Podium-slice exemption is N/A — no issue ticket was assigned; the previous session's slice-scoped capture was a slip, not a precedent).