...
> **Update 2026-07-02 (ADR-0032, [analysis](../analyses/adr-0032-project-defined-agent-reaction.md)) — engine decomposition landed + homelab flip live.** ADR-0016's `INFRA_PREAMBLE` engine constant is **superseded**: the engine no longer ships infra prose. `binding_type` is decomposed into per-binding capability flags (`scheduling`, `blocked_reconciler`, `worktree_default`) + a project-supplied `preamble:` file; no engine branch gates on `binding_type` anymore (it degrades to a thin preset of defaults). The `homelab` binding now sets `preamble: SYMPHONY.md` (authored in the homelab repo), `scheduling: true`, `blocked_reconciler: true`, `worktree_default: false` — behavior-neutral on deploy (flags equal the old infra defaults; only the preamble source changed from the engine constant to the project file). Deployed live via `symphony-host.service` restart onto `84304c3`. **The `INFRA_PREAMBLE`-renders-the-infra-prompt statement below is now historical.** See C-0356.
