...
The fix makes idle observable and gates the decision on two complementary signals. The initial cut (commit `9c058b7`) counted consecutive byte-unchanged pane captures, but pane stability alone is fragile: it is coupled to the Claude TUI redrawing its spinner/timer, was never validated live, and an idle session captured *empty* (alt-screen) when observed — so a working agent could be misjudged idle. Commit `dc09e61` adds the agent's session transcript as the trustworthy ground-truth signal: the jsonl that Claude appends on every conversation event, located via `session_continuity.session_file_path("claude", cwd, session_id)` and read by `_session_file_mtime` [source: claude_runner.py; source: session_continuity.py]. Idle now requires BOTH the pane AND the transcript mtime to stay unchanged across `IDLE_POLLS_BEFORE_NUDGE=30` one-second polls; the signals are complementary — a long tool call freezes the transcript while the pane animates, while an alt-screen pane captures static while the transcript grows — so a false-positive idle (and the run-killing fail-fast) requires both channels frozen at once, and a missing transcript counts as activity, never idle [source: claude_runner.py].

On idle with no done file it pastes a completion-protocol reminder into the live session (`_send_nudge` reuses `_paste_and_submit`, so the paste/Enter race handling carries over) up to `IDLE_NUDGE_ATTEMPTS=2` times; if still no done file it gives up early returning the same shape as the hard timeout — `AgentResult(-1, timed_out=True)` — plus a distinct `claude_idle_no_completion` log line, so downstream lands on the existing failed/blocked terminal path in ~90s instead of an hour [source: claude_runner.py; source: scheduler.py]. Idle is tracked with unchanged-signal counters rather than wall-clock, adding no `clock()` calls so the existing timing tests are unaffected; done-file and session-dead branches are unchanged and still checked first. Four tests cover nudge-then-complete, nudge-exhaustion failure, a changing pane never falsely nudged, and a static pane with an advancing transcript never falsely nudged [source: tests/test_claude_runner.py]. Headless `claude -p` (which would replace the whole sentinel protocol with a process-exit signal) was considered and rejected by the operator [source: wiki/raw/sessions/2026-06-14-claude-runner-idle-completion-nudge.md]. Live on `e1bc113`; the nudge/fail-fast path is not yet exercised by an organic live idle run.
