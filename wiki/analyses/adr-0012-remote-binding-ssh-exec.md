...
sources:
  ...
  - wiki/raw/sessions/2026-06-23-remote-claude-live-calibration.md
  - .kanban/issues/099-claudehost-seam-completion.md
  - .kanban/issues/100-route-runner-tmux-cleanup-through-host.md
  - .kanban/issues/101-remote-aware-launch-cwd-tempdir-pidfile.md
  - .kanban/issues/102-remote-modal-continuity-steering.md
...
Coverage now asserts both local byte-for-byte argv parity and remote SSH mediation: remote fake `SshClaudeHost` tests prove `has-session`, pane capture, `kill-session`, and cleanup route through SSH-wrapped tmux/rm commands rather than local tmux/filesystem calls [source: tests/test_claude_runner.py] [source: tests/test_claude_persist.py]. Verification passed exactly as issue-specified (`.venv/bin/python -m pytest tests/test_claude_runner.py tests/test_claude_persist.py tests/test_claude_host.py -q && /usr/local/bin/ruff check claude_runner.py claude_host.py`), touched-file LSP diagnostics were clean, and fresh review returned `RALPH_REVIEW: PASS` after inspecting `git diff b76db6cef32e4504cc9eb32d939f5b56d4702ad7 HEAD` [source: .kanban/issues/100-route-runner-tmux-cleanup-through-host.md] [source: .kanban/progress.md].

## Remote Claude v2 progress 2026-06-23 — Issues #101 and #102 remote launch + modal behavior

Issue #101 added host-injected remote launch support to `run_claude_agent`: remote launches use host-mediated tmux `new-session -c <remote-dir> -e SYMPHONY_ISSUE_ID=<id>`, skip local temp-dir creation, and skip local `/proc` pidfile registration; the local launch path still uses subprocess `cwd=` plus the `_claude_env` allowlist [source: claude_runner.py] [source: .kanban/issues/101-remote-aware-launch-cwd-tempdir-pidfile.md]. Verification passed with `tests/test_claude_runner.py`/`tests/test_claude_persist.py` and ruff, and fresh review returned `RALPH_REVIEW: PASS` [source: .kanban/progress.md].

Issue #102 made the remote poll loop viable without local transcript-mtime idle detection: permission/question modal detection now runs on every poll, remote Claude dispatch always cold-starts with `--session-id`, and `_deliver_steer_records` returns a no-op for remote hosts [source: claude_runner.py] [source: .kanban/issues/102-remote-modal-continuity-steering.md]. Coverage asserts early remote modal handling, `--session-id` despite resumed/transcript inputs, remote steering no-op, and unchanged local modal behavior; verification passed exactly as issue-specified (`.venv/bin/python -m pytest tests/test_claude_runner.py tests/test_claude_persist.py -q && /usr/local/bin/ruff check claude_runner.py`) with clean touched-file LSP diagnostics and fresh review `RALPH_REVIEW: PASS` [source: tests/test_claude_runner.py] [source: .kanban/progress.md].

Issue #103 opened the production dispatch path for remote Claude: `_apply_dispatch_gate` now allows remote+`claude` and skips the local Claude probe for remote Claude, `_prepare_resume_candidate` cold-refeeds remote Claude before local transcript eligibility, remote config accepts `default_agent: claude` while still forbidding remote `claude_persist` and non-coding bindings, `RoutingAgentAdapter` sends remote Claude to `ClaudeAgentAdapter`, and `main.build_binding_runtime` passes `remote` + `remote_repo_path` so `ClaudeAgentAdapter` constructs `SshClaudeHost` for `run_claude_agent` [source: scheduler/__init__.py] [source: config.py] [source: agent_runner.py] [source: claude_runner.py] [source: main.py] [source: .kanban/issues/103-scheduler-config-routing-wiring.md]. Verification passed with the issue command (`.venv/bin/python -m pytest tests/test_scheduler.py tests/test_config.py tests/test_agent_runner.py tests/test_main.py -q && /usr/local/bin/ruff check scheduler/__init__.py config.py agent_runner.py main.py`) plus the affected runner suite (`tests/test_claude_host.py tests/test_claude_runner.py tests/test_claude_persist.py tests/test_agent_runner.py tests/test_config.py tests/test_scheduler.py`, ruff, py_compile); the pi wave-audit retry passed with 0 critical / 0 warning / 1 note after an initial reviewer timeout, and the note was addressed with explicit remote+Claude config guard tests; logged in `plans/.feature-remote-claude-dispatch.state.yml` [source: tests/test_scheduler.py] [source: tests/test_config.py] [source: tests/test_agent_runner.py] [source: tests/test_main.py] [source: tests/test_remote_agent.py] [source: tests/test_claude_runner.py] [source: plans/.feature-remote-claude-dispatch.state.yml].

Issue #104 completed the attended production-runner calibration on `n8n`: a temporary `calibration-claude` Podium binding pointed at the disposable remote checkout `/tmp/symphony-calibration`, `ssh itadmin@100.95.224.218 tmux -V` reported `tmux 3.4`, and smoke Issues #115/#116 dispatched through the live scheduler into `ClaudeAgentAdapter`/`SshClaudeHost` [source: docs/adr/0012-remote-binding-ssh-exec.md] [source: plans/.feature-remote-claude-dispatch.state.yml]. Runs #324/#325 both exited 0 with `SYMPHONY_RESULT: done`, captured non-empty `result.0.txt` plus `done.0`, reported cwd `/tmp/symphony-calibration`, and recorded remote HEAD `7e4a319`; Run #325 observer samples showed first ready/prompt capture at ~4.8s, command output at ~9.6s, `result.0.txt` at ~13.0s, `done.0` at ~19.6s, and runner exit at 20.9s, with no timing constant changes needed [source: wiki/raw/sessions/2026-06-23-remote-claude-live-calibration.md] [source: plans/.feature-remote-claude-dispatch.state.yml]. Cleanup was verified: issue-specific remote temp dirs/sockets were gone, ControlMaster closed, and the disposable checkout stayed clean; scope is intentionally bounded to read-only smoke, so remote edit/commit landing was not exercised [source: plans/.feature-remote-claude-dispatch.state.yml]. ADR-0012 v2 is accepted [source: docs/adr/0012-remote-binding-ssh-exec.md].

Issue #105 closed the edit+commit landing gap with a lower-risk direct harness rather than a temporary production binding: a Python invocation of `ClaudeAgentAdapter(remote=RemotePolicy(...))` exercised `SshClaudeHost`/`run_claude_agent` against disposable n8n checkout `/tmp/symphony-105-remote-claude-20260624005402-26879`, and remote Claude wrote `symphony-remote-claude-smoke.txt` then committed `7f34e32 smoke: remote claude edit commit` [source: .kanban/issues/105-remote-claude-edit-commit-landing-smoke.md] [source: runs/105-remote-claude-edit-smoke.log]. Independent verification found `git status --short` empty, no issue-specific `symphony-claude-105-remote-edit-smoke-*` temp/tmux residue, and the disposable checkout was removed; no `bindings.yml` mutation or `symphony-host` restart was needed [source: .kanban/issues/105-remote-claude-edit-commit-landing-smoke.md] [source: runs/105-remote-claude-edit-smoke.log]. C-0314 tracks this as the edit/landing counterpart to #104's read-only calibration.
