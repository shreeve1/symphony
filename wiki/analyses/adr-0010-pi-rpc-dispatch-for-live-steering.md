...
Slice A (#050) closed dispatch parity; the full-path soak in C-0190 moved ADR-0010 to `accepted`. Live Steering is now partly implemented: #056 and #057 landed the channel and operator UI; #058 remains for remaining RPC lifecycle/ops hardening.
...
#047/#048/#049 stand (RPC-agnostic foundation). **#050 is done** (`PiRpcAgentAdapter`: dispatch parity + resume via `--session-id`; one-shot `PiAgentAdapter` retained as rollback, selected by `pi_mode`). #052 pi-side park parses `SYMPHONY_QUESTION` from the RPC final text (same marker contract as Claude). #053 tail stands (RPC persists the session jsonl when not `--no-session`). **#056 and #057 are done** (live steer channel + flyout UI). **#058 remains open** for remaining RPC lifecycle/ops hardening. Generalizes ADR-0001 and partially reverses the thin-engine "pi one-shot" posture.

## Claims

C-0178 (decision + parity spike + re-sequencing), C-0181..C-0183 for the landed #050 Slice A implementation, C-0190 for acceptance/full-path soak, and C-0193 for landed #056/#057 live steering channel + UI. Supersedes the "deferred live tmux send-keys steering (Claude-only)" clause of C-0176. See [CLAIMS.md](../CLAIMS.md).
