...
sources:
  ...
  - wiki/raw/sessions/2026-07-17-issue-464-remote-pi-duo-runtime-gap.md
  - wiki/raw/sessions/2026-07-17-issue-464-shell-environment-correction.md
  - wiki/raw/sessions/2026-07-17-issue-464-pi-login-fix-verified.md
  - /home/james/dotfiles/.pi/agent/extensions/pi-duo/extensions/pi-duo.ts
confidence: high
...
## Nested-provider runtime gap: remote Pi Duo (Issue 464)

Symphony's model gate validates the selected outer catalog tuple. For `pi-duo/Duo`, that does not prove Duo's configured actor and verifier models are usable in the shell environment that Symphony will launch. Pi Duo's `providerConfig()` deliberately catches actor lookup failure while building picker metadata and still registers the outer `Duo` virtual model with fallback metadata; only the request path calls fail-loud `lookupModel(config.actor)`. A host can therefore advertise and accept `pi-duo/Duo`, pass Symphony's dispatch gate, then exit immediately when Duo resolves its nested actor [source: /home/james/dotfiles/.pi/agent/extensions/pi-duo/extensions/pi-duo.ts] [source: wiki/raw/sessions/2026-07-17-issue-464-shell-environment-correction.md].

This occurred twice on remote `mac-dotfiles` Issue 464 (Runs 2687 and 2692). The key distinction was shell initialization, not an absent host model: the Mac login-interactive shell had the MiniMax credential available and listed `minimax/MiniMax-M3`, so direct terminal invocations worked; Symphony launched the same Pi binary/version through noninteractive SSH, where the credential was absent and MiniMax models were omitted. Run 2692 consequently exited 1 after 7.5 seconds and the scheduler correctly classified it as `nonzero` → `blocked`. The accompanying `zai/glm-5.2` warning had the same noninteractive-environment shape but was secondary because Duo's configured verifier was CLIProxy, not ZAI [source: wiki/raw/sessions/2026-07-17-issue-464-shell-environment-correction.md].

`run_remote_agent()` explicitly documents that remote SSH dispatch does not source `.zshrc`/`.bashrc`. `_remote_exports()` forwards Symphony callback variables, not arbitrary provider credentials, then `_build_remote_command()` executes `pi --mode rpc` directly. Operational implication: remote composite-provider smoke tests must use Symphony's noninteractive SSH environment; a successful command in an interactive terminal does not prove dispatch readiness [source: agent_runner.py] [source: wiki/raw/sessions/2026-07-17-issue-464-shell-environment-correction.md].

The operator repaired Issue 464's authentication gap through Pi's supported `/login` flow on the Mac. After entering the key there, batch-mode SSH listed `minimax/MiniMax-M3`, the exact noninteractive print command returned `pong`/exit 0, and a direct `--mode rpc` probe also exited 0. This proves Pi's own auth store supplies provider credentials to noninteractive dispatch; no `.zshenv` credential loader, Duo change, model-catalog change, or Symphony restart is required for this incident [source: wiki/raw/sessions/2026-07-17-issue-464-pi-login-fix-verified.md].
