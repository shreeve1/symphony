...
- **Steering** (live mid-run) — operator input injected into a *running* Run, distinct from the between-Run Question Park reply loop. ADR-0010 first implemented the pi RPC path through #056/#057/#058: the API writes transient steer/abort queue records plus durable `Operator Steer` / `Operator Abort` comment blocks, the pi RPC adapter forwards queue records to stdin, the flyout Session tab exposes steer/abort controls for active pi RPC runs, and #058 clears per-run/stale steer queue files on adapter exit and startup reaping. Persistent Claude sessions later gained their own tmux delivery path (#079), and #083 updated the API to allow Claude steer/abort only for bindings with `claude_persist: true`; non-persist Claude runs return `409` with `enable claude_persist for live Claude steering`. [source: CONTEXT.md] [source: docs/adr/0010-pi-rpc-dispatch-for-live-steering.md] [source: web/api/main.py#L1315-L1330] [source: web/api/steer_queue.py] [source: agent_runner.py] [source: claude_runner.py] [source: web/api/tests/test_steer.py#L101-L139] [source: .kanban/issues/058-rpc-lifecycle-ops.md] [source: .kanban/issues/083-api-allow-claude-steer.md]
- Deferred (no issues): `--fork` A/B exploration. (Live mid-run steering is no longer deferred: pi uses RPC via ADR-0010, and local persistent Claude uses `claude_persist` tmux steering; non-persist Claude still uses park-and-reply.)

## Backlog

`.kanban/issues/047`–`055` plus ADR-0010 steering/RPC follow-ups. Status: 047 (run columns), 048 (decision core), 049 (delta renderer), **050 (pi RPC dispatch + resume wiring)**, **051 (Claude resume wiring)**, **052 (Question Park)**, **053 (Live Session Tail)**, **054 (Fast re-dispatch)**, **055 (Checkpointed exploration)**, **056 (Live Steering channel)**, **057 (flyout Steering UI)**, **058 (pi RPC lifecycle/ops hardening)**, and **083 (API allow Claude steer for `claude_persist`)** are done. [source: .kanban/issues/047-run-session-tracking-columns.md] [source: .kanban/issues/048-continuity-decision-core.md] [source: .kanban/issues/049-delta-only-resume-prompt.md] [source: .kanban/issues/050-pi-resume-end-to-end.md] [source: .kanban/issues/051-claude-resume-end-to-end.md] [source: .kanban/issues/052-question-park.md] [source: .kanban/issues/053-live-session-tail.md] [source: .kanban/issues/054-fast-redispatch-on-reply.md] [source: .kanban/issues/055-checkpointed-exploration.md] [source: .kanban/issues/056-live-steer-channel.md] [source: .kanban/issues/057-steer-ui-flyout.md] [source: .kanban/issues/058-rpc-lifecycle-ops.md] [source: .kanban/issues/083-api-allow-claude-steer.md]
...
C-0175, C-0176, C-0177, C-0178, C-0180, C-0181, C-0182, C-0183, C-0184, C-0185, C-0186, C-0187, C-0192, C-0193, C-0194, C-0211, and C-0239 in [CLAIMS.md](../CLAIMS.md).
