---
title: Session Resume continuity (design-stage)
type: concept
status: promoted
created: 2026-06-13
updated: 2026-06-13
...
confidence: high
tags: [session-resume, continuity, re-feed, question-park, session-tail, design-stage, podium]
---

# Session Resume continuity

> **Design-stage, unimplemented as of 2026-06-13.** Live continuity is still pure text re-feed (see [operator-reply](operator-reply.md)). This page records the agreed design (ADR-0009) and its backlog; it is NOT a description of current running behavior.
...
- **Derived id** `UUIDv5(namespace, issue.id)` — the Issue is the session key; nothing stored to drift. pi `--session-id`; Claude `--session-id` (create) / `--resume` (resume), branch by filesystem probe of the cwd-namespaced session file. Stays on the tmux/CLI path (ADR-0001), not the Agent SDK.
- **cwd coupling** — sessions are namespaced by working directory (Claude `~/.claude/projects/<encoded-cwd>/<id>.jsonl`; pi `~/.pi/agent/sessions/<cwd-slug>/`). Resume only works when cwd is stable+present.
- **Eligibility predicate** (all four, else re-feed): same agent kind ∧ cwd present+unchanged ∧ session file present ∧ git HEAD unchanged since the session last ran (`run.agent_session_sha`). Scope: in_review/blocked reply loop only; Done-reopen falls back; worktree lifecycle (#021) untouched.
...
`.kanban/issues/047`–`055`. Sequence: 047 (run columns) → {048 decision core, 049 delta renderer} → 050 (pi e2e) → 051 (Claude e2e) → {052 Question Park → 055 checkpointed, 053 Session Tail}; 054 fast re-dispatch parallel after 047.
...
C-0175, C-0176 in [CLAIMS.md](../CLAIMS.md).
