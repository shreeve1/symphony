...
- **Failed-migration recovery is operator-initiated, not auto-retried.** A failed migration leaves `podium-migrations.service` in `failed` state (RemainAfterExit=yes keeps the failed result visible). The next `systemctl start podium-api.service` does NOT re-pull a failed dependency's `ExecStart=` (systemd's `Wants=` only triggers Start on inactive-but-not-failed). Recovery is `systemctl reset-failed podium-migrations.service && systemctl start podium-migrations.service` followed by `systemctl restart podium-api.service`. Documented in `web/README.md`'s Migrations section and `wiki/concepts/symphony-operations.md`'s Boot Ordering subsection.
- **Pre-migration DB backups created ad-hoc (e.g. `podium.db.pre023.bak-…`) are now auto-ignored by `.gitignore`** rules `podium.db.*.bak-*` / `podium.db.pre*.bak`, so the canonical backup path stays `/backup/podium/` (`podium-backup.sh` cron) rather than ad-hoc blobs in the repo root.
