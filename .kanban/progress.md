...
**Notes for next iteration:** Issue #072 is implementation-only; it is left `in-progress` for review. No downstream issue is blocked.
**Verification:** `uv run pytest` (887 passed, 2 skipped), `uv run ruff check agent_runner.py claude_runner.py`, `uv run python -m py_compile agent_runner.py claude_runner.py`, touched-file LSP diagnostics clean.
