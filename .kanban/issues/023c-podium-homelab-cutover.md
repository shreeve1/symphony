...
- [ ] Alembic revision adds `approval_required`, `approved`, `scheduled_for` to `issue`; `alembic upgrade head` succeeds against a fresh DB.
- [ ] `PodiumTrackerAdapter` projects the three infra Roles onto the new columns; `tests/test_tracker_podium_infra.py` covers each.
- [ ] `bindings.yml` for `homelab` declares `tracker: podium`; the Plane block is preserved commented for rollback.
- [ ] `systemctl restart symphony-host.service` succeeds (operator-approved at the moment).
- [ ] Smoke ticket filed via Podium UI dispatches a real run against `/home/james/homelab/`; Run row captures verdict + log path; comments/context populated.
- [ ] `CONTEXT.md` updated: Mode → historical; Run Worktree rewritten; Landing rewritten; Plane-retired note under Tracker Adapter; flagged ambiguities pruned.
- [ ] No writes to the homelab Plane project after cutover (verified by capturing `plane_adapter` calls in a test against the cutover binding).
- [ ] Rollback documented: revert bindings.yml edit + restart. Captured in `web/README.md`.
