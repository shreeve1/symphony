...
- Launch `pi --mode rpc --provider <p> --model <m> [--skill <dir>] --session-id <derive_session_id(issue.id)>` from the bound repo cwd. NOT `--no-session` (the session must persist for resume + #053 tail), NOT `--continue` (silent-fresh hazard).
