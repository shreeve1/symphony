...
1. Resolve binding inputs:
   ...
   - `remote_host` / `remote_user` — optional; set both to make this a remote binding (ADR-0012). Writes a `remote:` block to `bindings.yml`, dispatches over SSH, and surfaces the frontend "name — repo" label automatically. `remote_identity` is an optional SSH key path. Remote bindings require `binding_type=coding`, `default_agent=pi`, `pi_mode=one-shot` (v1 constraints, mirrored from `config.py`); the scaffold raises `ValueError` otherwise.
2. Run `scaffold_podium_binding(...)`. It takes a `PodiumBindingScaffoldRequest` plus required keyword-only `db_path` and `bindings_path`:
   ...
   result = scaffold_podium_binding(
       PodiumBindingScaffoldRequest(
           ...
           # Remote binding (ADR-0012): set host+user, require one-shot pi coding.
