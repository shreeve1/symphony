...
4. Poll `GET /api/issues/{issue_id}/runs` until a Run row appears or the timeout expires. The smoke Issue is created with `approval.enabled=false` on most bindings, so the live scheduler **auto-dispatches a real agent run** against the target repo (creates a worktree/branch). This is not read-only.
5. Fetch the final Run with `GET /api/runs/{run_id}` when needed.
6. Report Issue id, Run id, state, verdict, and summary. **The Run's `agent` (`pi`/`claude`) is the harness; its `provider`/`model` come from the agent's own config and may differ from what the binding implies** (e.g. a `pi` binding dispatched on `openai-codex` / `gpt-5.5:high`). Surface provider/model in the report and flag if it is not what James expected.
