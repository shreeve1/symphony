...
5. **Review phase is gated on `auto_land` (ADR-0023, scoped by issue #149).** `tracker_podium.py:list_candidates` fires a second (review) run **only for slicer-authored issues with `auto_land=true`**. A smoke Issue is operator-created and `auto_land=false` (safety rule below), so it gets **one implement run** and then parks in `in_review` (manual merge gate) — **no second run**. Expect two Run rows only for slicer/`auto_land` issues; for a smoke, stop polling once the single implement run is terminal (`latest_run_state=succeeded`, verdict `done`). `poll_podium_issue_run` stops at the first run, which is exactly right for an operator smoke.
6. Fetch the final Run with `GET /api/runs/{run_id}` when needed.
7. Report Issue id, Run ids (implement, plus review if one fired), final state, verdict, and summary. **The Run's `agent` (`pi`/`claude`) is the harness; its `provider`/`model` come from the agent's own config and may differ from what the binding implies** (e.g. a `pi` binding dispatched on `openai-codex` / `gpt-5.5:high`). Surface provider/model in the report and flag if it is not what James expected.
...
- **Never set `auto_land=true` on a smoke Issue.** A smoke Issue is operator-created and leaves `auto_land` at its `false` default. With `auto_land=false`, no review run fires (step 5) and the single implement run parks the Issue in `in_review` for a manual merge. Setting `auto_land=true` would instead trigger the unattended review phase, which auto-merges throwaway smoke work into `main` via `land_worktree` — exactly what must not happen.
